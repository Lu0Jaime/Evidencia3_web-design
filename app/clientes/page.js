...
export default function ClientesPage() {
  ...
  function iniciarEdicion(c) {
    ...
    setEditId(c.id_cliente); setErrors({})
    ...
    <div className="page-wrap">
      <div className="page-header">
        <div className="page-title" style={{ display:'flex', alignItems:'center', gap:8 }}>
          <IconUsers size={20} /> Gestión de Clientes
        </div>
        ...
        <div className="card-header">
          ...
          {editId && (
            <button className="btn btn-secondary btn-sm" style={{marginLeft:'auto'}} onClick={() => { setForm(BLANK); setEditId(null); setErrors({}) }}>
              <IconX size={13} /> Cancelar edición
            </button>
          )}
        </div>
        <div className="card-body">
          ...
          <div className="btn-group">
            <button className="btn btn-primary" onClick={guardar} disabled={saving}>
              {saving ? <><span className="spinner"></span> Guardando…</> : <><IconSave size={14} /> {editId ? 'Actualizar' : 'Registrar'} Cliente</>}
            </button>
            {!editId && (
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              <p style={{color:'var(--text2)',fontSize:'.85rem',lineHeight:1.6}}>
                ...
                <span style={{color:'var(--red)',fontSize:'.8rem'}}>Se eliminarán también todas sus obligaciones fiscales.</span>
              </p>
            </div>
      ...
      {detail && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setDetail(null)}>
          <div className="modal" style={{maxWidth:560}}>
            <div className="modal-header">
              <div className="modal-title" style={{display:'flex',alignItems:'center',gap:8}}><IconUser size={16} /> {detail.nombre_cliente}</div>
              <button className="modal-close" onClick={()=>setDetail(null)}><IconX size={16} /></button>
            </div>
            <div className="modal-body">
