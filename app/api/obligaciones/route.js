...
function calcEstatusVisual(fechaStr, cumplimiento) {
  const hoy   = new Date(); hoy.setHours(0,0,0,0)
  const fecha = new Date(fechaStr + 'T00:00:00')
  const dias  = Math.round((fecha - hoy) / 86400000)
  if (cumplimiento === 'Presentada') return { estatus_visual: 'Al día', dias_restantes: dias }
...
export async function GET(request) {
  try {
    ...
    let enriched = (data || []).map(o => {
      const calc = calcEstatusVisual(o.fecha_vencimiento, o.estatus_cumplimiento)
      return { ...o, ...calc }
    })
...
export async function POST(request) {
  try {
    ...
    const { estatus_visual, dias_restantes } = calcEstatusVisual(fecha_vencimiento, estatus_cumplimiento)
